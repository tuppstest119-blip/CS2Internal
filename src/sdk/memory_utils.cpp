...
namespace sdk {
    ...
    std::uint8_t* find_pattern(const char* module_name, const char* pattern) {
        ...
        logger::DebugLog("[MEM] find_pattern: module=%s handle=0x%p", module_name, module_handle);
        if (module_handle == nullptr) {
            logger::DebugLog("[MEM] find_pattern: failed to get handle for module %s", module_name);
