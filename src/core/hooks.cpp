...
HRESULT __stdcall hook_present(IDXGISwapChain* swap_chain, UINT sync_interval, UINT flags) {
    ...
    logger::DebugLog("[HOOK] Present called");

    if (!interfaces::d3d11_render_target_view) {
        interfaces::create_render_target();
        logger::DebugLog("[HOOK] Created render target");
    }
    ...
    // ImGui-Initialisierung sicher im Render-Thread
    if (!imgui_initialized && interfaces::d3d11_device && interfaces::d3d11_device_context && interfaces::hwnd) {
        ImGui::CreateContext();
        ImGui_ImplWin32_Init(interfaces::hwnd);
        ImGui_ImplDX11_Init(interfaces::d3d11_device, interfaces::d3d11_device_context);
        ImGui::StyleColorsDark();
        imgui_initialized = true;
        logger::DebugLog("[HOOK] ImGui initialized");
    }
...
HRESULT __stdcall hook_resize_buffers(IDXGISwapChain* swap_chain, UINT buffer_count, UINT width, UINT height, DXGI_FORMAT new_format, UINT swap_chain_flags) {
    logger::DebugLog("[HOOK] ResizeBuffers called: %ux%u", width, height);
    ...
    if (SUCCEEDED(result)) {
        ...
        logger::DebugLog("[HOOK] ResizeBuffers succeeded");
    }
    return result;
}
...
bool __fastcall hook_mouse_input_enabled(sdk::interface_csgo_input* csgo_input) {
    return globals::IsMenuOpened() ? false : hooks::original_mouse_input_enabled(csgo_input);
}

void* __fastcall hook_set_relative_mouse_mode(sdk::interface_input_system* input_system, bool enabled) {
    globals::SetRelativeMouseMode(enabled);
    return hooks::original_set_relative_mouse_mode(input_system, globals::IsMenuOpened() ? false : enabled);
}
...
namespace hooks {
    void create() {
        ...
        if (MH_CreateHook(sdk::virtual_function_get<void*, 8>(interfaces::swap_chain_dx11->swap_chain), &hook_present, reinterpret_cast<void**>(&original_present)) != MH_OK) {
            throw std::runtime_error("failed to create present hook");
        }

        if (MH_CreateHook(sdk::virtual_function_get<void*, 13>(interfaces::swap_chain_dx11->swap_chain), &hook_resize_buffers, reinterpret_cast<void**>(&original_resize_buffers)) != MH_OK) {
            throw std::runtime_error("failed to create resize buffers hook");
        }

        {
            IDXGIDevice* dxgi_device = nullptr;
            if (FAILED(interfaces::d3d11_device->QueryInterface(__uuidof(IDXGIDevice), reinterpret_cast<void**>(&dxgi_device)))) {
            ...
            if (MH_CreateHook(sdk::virtual_function_get<void*, 10>(dxgi_factory), &hook_create_swap_chain, reinterpret_cast<void**>(&original_create_swap_chain)) != MH_OK) {
        ...
        if (MH_CreateHook(sdk::virtual_function_get<void*, 15>(interfaces::csgo_input), &hook_mouse_input_enabled, reinterpret_cast<void**>(&original_mouse_input_enabled)) != MH_OK) {
            throw std::runtime_error("failed to create mouse input enabled hook");
        }

        if (MH_CreateHook(sdk::virtual_function_get<void*, 76>(interfaces::input_system), &hook_set_relative_mouse_mode, reinterpret_cast<void**>(&original_set_relative_mouse_mode)) != MH_OK) {
